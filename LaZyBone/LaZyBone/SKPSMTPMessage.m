...
@synthesize login, pass, relayHost, relayPorts, subject, fromEmail, toEmail, parts, requiresAuth, inputString, wantsSecure, connectTimer, connectTimeout, watchdogTimer, validateSSLChain;
@synthesize delegate;
...
- (BOOL)send
{
    ...
    [NSStream getStreamsToHostNamed:relayHost port:relayPort inputStream:&is outputStream:&os];
            ...
            switch (sendState)
            {
                case kSKPSMTPConnecting:
                {
                    if ([tmpLine hasPrefix:@"220 "])
                    {
                        ...
                        NSLog(@"C: %@", ehlo);
                        CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[ehlo UTF8String], [ehlo lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
                ...
                case kSKPSMTPWaitingEHLOReply:
                {
                    ...
                    else if ([tmpLine hasPrefix:@"250-STARTTLS"] && !isSecure && wantsSecure)
                    {
                        ...
                        NSLog(@"C: %@", startTLS);
                        CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[startTLS UTF8String], [startTLS lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
                        [self startShortWatchdog];
                    }
                    else if ([tmpLine hasPrefix:@"250 "])
                    {
                        if (self.requiresAuth)
                        {
                            ...
                            if (serverAuthPLAIN)
                            {
                                ...
                                NSLog(@"C: %@", authString);
                                CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[authString UTF8String], [authString lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
                                [self startShortWatchdog];
                            }
                        ...
                        else
                        {
                            ...
                            NSLog(@"C: %@", mailFrom);
                            CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[mailFrom UTF8String], [mailFrom lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
                            [self startShortWatchdog];
                        }
                    }
                    break;
                ...
                case kSKPSMTPWaitingTLSReply:
                {
                    if ([tmpLine hasPrefix:@"220 "])
                    {
                        ...
                        if (!self.validateSSLChain)
                        {
                            ...
                            CFReadStreamSetProperty((__bridge CFReadStreamRef)inputStream, kCFStreamSSLValidatesCertificateChain, kCFBooleanFalse);
                            CFWriteStreamSetProperty((__bridge CFWriteStreamRef)outputStream, kCFStreamSSLValidatesCertificateChain, kCFBooleanFalse);
                        }
                ...
                case kSKPSMTPWaitingAuthSuccess:
                {
                    if ([tmpLine hasPrefix:@"235 "])
                    {
                        ...
                        NSLog(@"C: %@", mailFrom);
                        CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[mailFrom cStringUsingEncoding:NSASCIIStringEncoding], [mailFrom lengthOfBytesUsingEncoding:NSASCIIStringEncoding]);
                        [self startShortWatchdog];
                    }
                ...
                case kSKPSMTPWaitingFromReply:
                {
                    if ([tmpLine hasPrefix:@"250 "])
                    {
                        ...
                        NSLog(@"C: %@", rcptTo);
                        CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[rcptTo UTF8String], [rcptTo lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
                        [self startShortWatchdog];
                    }
                    break;
                }
                case kSKPSMTPWaitingToReply:
                {
                    if ([tmpLine hasPrefix:@"250 "])
                    {
                        ...
                        NSLog(@"C: %@", dataString);
                        CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[dataString UTF8String], [dataString lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
                        [self startShortWatchdog];
                    }
                ...
                case kSKPSMTPWaitingSendSuccess:
                {
                    if ([tmpLine hasPrefix:@"250 "])
                    {
                        ...
                        NSLog(@"C: %@", quitString);
                        CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[quitString UTF8String], [quitString lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
                        [self startShortWatchdog];
                    }
...
- (void)sendParts
{
    ...
    CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[messageData bytes], [messageData length]);
    ...
    for (NSDictionary *part in parts)
    {
        ...
        NSLog(@" ????????Printing here%@",[part objectForKey:kSKPSMTPPartMessageKey]);
    ...
    //NSLog(@"C: %@", message);
    CFWriteStreamWriteFully((__bridge CFWriteStreamRef)outputStream, (const uint8_t *)[message UTF8String], [message lengthOfBytesUsingEncoding:NSUTF8StringEncoding]);
